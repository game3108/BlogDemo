...
static force_inline NSNumber *YYNSNumberCreateFromID(__unsafe_unretained id value) {
    ...
    if ([value isKindOfClass:[NSString class]]) {
        ...
        if ([(NSString *)value rangeOfCharacterFromSet:dot].location != NSNotFound) {       //判断是否包含"."
            const char *cstring = ((NSString *)value).UTF8String;                           //包含说明有小数，用atof
            if (!cstring) return nil;
            double num = atof(cstring);
            if (isnan(num) || isinf(num)) return nil;
            return @(num);
        } else {
            const char *cstring = ((NSString *)value).UTF8String;                           //不包含小数情况
            if (!cstring) return nil;
...
//根据keypath list获取dictionary的value，不停递归往里获得value
...
//model到 json object (NSArray/NSDictionary/NSString/NSNumber/NSNull)
